        ...
        <section class="as-page__panel">
          <div class="as-page__table-wrap">
            <table class="as-page__table">
              <thead><tr><th>Date</th><th>Assessments</th><th>Provider</th><th>Cache</th><th>Blacklist</th><th>Tor</th><th>Quota skips</th><th>Events</th></tr></thead>
              <tbody>{{#each @controller.data.daily as |day|}}<tr><td>{{day.stat_date}}</td><td>{{day.assessments}}</td><td>{{day.provider_calls}}</td><td>{{day.cache_hits}}</td><td>{{day.local_blacklist_hits}}</td><td>{{day.tor_hits}}</td><td>{{day.quota_skips}}</td><td>{{day.events_created}}</td></tr>{{/each}}</tbody>
            </table>
          </div>
        </section>
      {{else}}
