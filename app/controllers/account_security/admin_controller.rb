    ...
    def serialize_correlation(item, review_history: nil, policy_action_state: nil)
      ...
      shared_ip_details = serialize_shared_ip_details(evidence["shared_ip_details"])
      {
        ...
        evidence: {
          ...
          core_auth_history_complete: evidence["core_auth_history_complete"] == true,
          exact_ip_population_complete: evidence["exact_ip_population_complete"] == true,
          ...
          shared_independent_network_count: evidence["shared_independent_network_count"].to_i,
          shared_exact_ip_network_overlap_count: evidence["shared_exact_ip_network_overlap_count"].to_i,
          shared_independent_networks: Array(evidence["shared_independent_networks"]).map(&:to_s).first(AccountCorrelationService::MAX_SHARED_NETWORKS_IN_PAYLOAD),
          max_independent_shared_network_users: evidence["max_independent_shared_network_users"].to_i,
          ...
          temporal_within_6h_count: evidence["temporal_within_6h_count"].to_i,
          temporal_within_24h_count: evidence["temporal_within_24h_count"].to_i,
          temporal_within_72h_count: evidence["temporal_within_72h_count"].to_i,
          ...
          temporal_public_within_7d_count: evidence["temporal_public_within_7d_count"].to_i,
          ...
          max_temporal_ip_users_24h: nonnegative_integer_or_nil(evidence["max_temporal_ip_users_24h"]),
          max_temporal_ip_users_7d: nonnegative_integer_or_nil(evidence["max_temporal_ip_users_7d"]),
          ...
          auth_proximity_within_1h_count: evidence["auth_proximity_within_1h_count"].to_i,
          auth_proximity_within_6h_count: evidence["auth_proximity_within_6h_count"].to_i,
          auth_proximity_within_24h_count: evidence["auth_proximity_within_24h_count"].to_i,
          auth_proximity_within_72h_count: evidence["auth_proximity_within_72h_count"].to_i,
          auth_proximity_within_7d_count: evidence["auth_proximity_within_7d_count"].to_i,
          ...
          auth_proximity_public_ip_within_24h_count: evidence["auth_proximity_public_ip_within_24h_count"].to_i,
          auth_proximity_public_ip_within_7d_count: evidence["auth_proximity_public_ip_within_7d_count"].to_i,
          ...
          public_ip_transition_pattern_count: evidence["public_ip_transition_pattern_count"].to_i,
          public_ip_transition_closest_gap_seconds: nonnegative_integer_or_nil(evidence["public_ip_transition_closest_gap_seconds"]),
          aligned_public_ip_transition_1h_count: evidence["aligned_public_ip_transition_1h_count"].to_i,
          aligned_public_ip_transition_6h_count: evidence["aligned_public_ip_transition_6h_count"].to_i,
          aligned_public_ip_transition_24h_count: evidence["aligned_public_ip_transition_24h_count"].to_i,
          aligned_public_ip_transition_3d_count: evidence["aligned_public_ip_transition_3d_count"].to_i,
          aligned_public_ip_transition_7d_count: evidence["aligned_public_ip_transition_7d_count"].to_i,
          aligned_public_ip_transition_30d_count: evidence["aligned_public_ip_transition_30d_count"].to_i,
          aligned_public_ip_transition_90d_count: evidence["aligned_public_ip_transition_90d_count"].to_i,
          aligned_public_ip_transition_180d_count: evidence["aligned_public_ip_transition_180d_count"].to_i,
          public_ip_transition_beyond_180d_count: evidence["public_ip_transition_beyond_180d_count"].to_i,
          public_ip_transition_unaligned_count: evidence["public_ip_transition_unaligned_count"].to_i,
          public_ip_transition_population_complete: evidence["public_ip_transition_population_complete"] == true,
          max_public_ip_transition_users: nonnegative_integer_or_nil(evidence["max_public_ip_transition_users"]),
          max_public_ip_transition_users_24h: nonnegative_integer_or_nil(evidence["max_public_ip_transition_users_24h"]),
    ...
    def serialize_temporal_ip_details(value)
      Array(value).first(TemporalCorrelationEvidence::MAX_DETAILS).filter_map do |raw|
...

        {
          ...
          temporal_population_complete: raw["temporal_population_complete"] == true,
          temporal_population_users_24h: nonnegative_integer_or_nil(raw["temporal_population_users_24h"]),
          temporal_population_users_7d: nonnegative_integer_or_nil(raw["temporal_population_users_7d"]),
          temporal_population_users_30d: nonnegative_integer_or_nil(raw["temporal_population_users_30d"]),
        }
      end
    end
    ...
    def serialize_auth_proximity_details(value)
      Array(value).first(TemporalCorrelationEvidence::MAX_AUTH_PROXIMITY_DETAILS).filter_map do |raw|
...

        {
          ...
          within_1h_count: raw["within_1h_count"].to_i.clamp(0, 100_000),
          within_6h_count: raw["within_6h_count"].to_i.clamp(0, 100_000),
          within_24h_count: raw["within_24h_count"].to_i.clamp(0, 100_000),
          within_72h_count: raw["within_72h_count"].to_i.clamp(0, 100_000),
          within_7d_count: raw["within_7d_count"].to_i.clamp(0, 100_000),
    ...
    def serialize_public_ip_transition_details(value)
      Array(value).first(TemporalCorrelationEvidence::MAX_TRANSITION_DETAILS).filter_map do |raw|
...

        {
          ...
          transition_population_complete: raw["transition_population_complete"] == true,
          transition_user_count: nonnegative_integer_or_nil(raw["transition_user_count"]),
          transition_user_count_24h: nonnegative_integer_or_nil(raw["transition_user_count_24h"]),
          transition_user_count_7d: nonnegative_integer_or_nil(raw["transition_user_count_7d"]),
        }
      end
    end
