    ...
    def execute(_args)
      ...
      stale_event_ids =
        ::AccountSecurity::RiskEvent
          ...
          .select(:id)
      delete_in_batches(::AccountSecurity::ProviderReport.where(risk_event_id: stale_event_ids))
      if defined?(::AccountSecurity::RiskEventAudit)
        delete_in_batches(::AccountSecurity::RiskEventAudit.where(risk_event_id: stale_event_ids))
      end
      delete_in_batches(::AccountSecurity::RiskEvent.where(id: stale_event_ids))
      delete_in_batches(
        ::AccountSecurity::ProviderReport.where.not(
          risk_event_id: ::AccountSecurity::RiskEvent.select(:id),
        ),
      )
      if defined?(::AccountSecurity::RiskEventAudit)
        delete_in_batches(
          ::AccountSecurity::RiskEventAudit.where.not(
            risk_event_id: ::AccountSecurity::RiskEvent.select(:id),
          ),
        )
      end
